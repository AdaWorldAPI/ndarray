...
// ============================================================================
// Float polyfills — under no_std, libm provides sqrt/powi.
// ============================================================================
...
pub mod consts {
    use std::f64::consts::{GOLDEN_RATIO, SQRT_2, TAU};
...
pub fn extract_slices(data: &[i8; 34]) -> ([f64; 8], f64, f64) {
    ...
    for (i, slot) in slices.iter_mut().enumerate() {
        ...
        *slot = i32::from_le_bytes(bytes) as f64;
    }
...
pub fn extract_7plus1(data: &[i8; 34]) -> ([f64; 8], f64, f64, f64) {
    ...
    let mut slices = [0.0f64; 8];
    for (i, slot) in slices.iter_mut().enumerate().take(7) {
        let start = 1 + (i * 4);
        let bytes = [data[start] as u8, data[start + 1] as u8, data[start + 2] as u8, data[start + 3] as u8];
        *slot = i32::from_le_bytes(bytes) as f64;
    }
...
pub mod seven_plus_one {
    ...
    pub fn to_clam48(m: &Manifold, threshold: f64, max_contra: f64) -> [u8; 6] {
        ...
        let contra_fires = if m.contradiction.abs() > max_contra * 0.5 {
            1u8
        } else {
            0u8
        };
        ...
        let peak_mag = super::fsqrt(peak_mag2);

        // B3: peak lane (3 bits) + X sign (1 bit) + 4 bits X fractional
        let x_sign = if m.x[peak_lane] >= 0.0 { 0u8 } else { 1u8 };
        let b3 = ((peak_lane as u8) << 5) | (x_sign << 4) | ((m.x[peak_lane].abs().min(15.0) as u8) & 0x0F);
...
pub mod dead_zone {
    ...
    pub fn flip_bit(data: &[i8; 34], bit_pos: usize) -> [i8; 34] {
        ...
        corrupted[byte_idx] ^= 1i8 << bit_idx;
        corrupted
    }
    ...
    pub fn correlation(a: &seven_plus_one::Manifold, b: &seven_plus_one::Manifold) -> f64 {
        ...
        vals_a[..7].copy_from_slice(&a.x[..7]);
        vals_a[7..14].copy_from_slice(&a.y[..7]);
        vals_b[..7].copy_from_slice(&b.x[..7]);
        vals_b[7..14].copy_from_slice(&b.y[..7]);
        ...
        let denom = super::fsqrt(var_a * var_b);
        if denom < 1e-15 {
            0.0
        } else {
            cov / denom
    ...
    pub fn run_benchmark(seed: &[i8; 34], _threshold: f64) -> Vec<(usize, f64, &'static str)> {
...
pub mod uniformity {
    ...
    pub fn gap_cv(gaps: &[f64]) -> f64 {
        ...
        let variance = gaps.iter().map(|g| super::fpowi(g - mean, 2)).sum::<f64>() / n;
        super::fsqrt(variance) / mean
    }
...
mod tests {
    ...
    fn verify_trig_constants() {
        for n in 0..8 {
            ...
            assert!(cos_err < 1e-12, "cos[{n}]: expected {cos_expected}, got {}, err={cos_err}", consts::COS_GOLDEN[n]);
            assert!(sin_err < 1e-12, "sin[{n}]: expected {sin_expected}, got {}, err={sin_err}", consts::SIN_GOLDEN[n]);
        }
    }
    ...
    fn verify_radii_gamma() {
        for n in 0..8 {
            ...
            assert!(err < 1e-12, "radius_gamma[{n}]: expected {expected}, got {}, err={err}", consts::RADII_GAMMA[n]);
        }
    }
    ...
    fn verify_lane0_singularity() {
        ...
        // √(n+γ): Lane 0 lives
        const { assert!(consts::RADII_GAMMA[0] > 0.75) };
    ...
    fn gamma_more_uniform_than_sqrt() {
        ...
        assert!(cv_gamma < cv_sqrt, "Euler-gamma should produce more uniform gaps");
        ...
        assert!(ratio_gamma < ratio_sqrt, "Euler-gamma should reduce max/min gap ratio");
    }
    ...
    fn make_test_seed() -> [i8; 34] {
        ...
        for (i, slot) in seed.iter_mut().enumerate().take(33).skip(1) {
    ...
    fn flat8_basic() {
        ...
        for (i, val) in encoded.iter().enumerate() {
            assert!(val.abs() > 1.0, "Lane {i} too small");
        }
    ...
    fn spiral8_lane0_dead() {
        ...
        assert!(x[0].abs() < 1e-10 && y[0].abs() < 1e-10, "Lane 0 should be dead in spiral8");
    }
    ...
    fn clam48_roundtrip() {
        ...
        eprintln!(
            "CLAM-48: [{:02X} {:02X} {:02X} {:02X} {:02X} {:02X}]",
    ...
    fn dead_zone_structure() {
        ...
        assert_eq!(contra_mean, 0.0, "Contradiction errors must produce zero manifold displacement");
    }
