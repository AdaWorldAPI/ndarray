...
extern "C" {
    ...
    fn cblas_sgemm(
        layout: c_int, transa: c_int, transb: c_int, m: c_int, n: c_int, k: c_int, alpha: c_float, a: *const c_float,
        lda: c_int, b: *const c_float, ldb: c_int, beta: c_float, c: *mut c_float, ldc: c_int,
    );
    fn cblas_dgemm(
        layout: c_int, transa: c_int, transb: c_int, m: c_int, n: c_int, k: c_int, alpha: c_double, a: *const c_double,
        lda: c_int, b: *const c_double, ldb: c_int, beta: c_double, c: *mut c_double, ldc: c_int,
    );
    fn cblas_sgemv(
        layout: c_int, trans: c_int, m: c_int, n: c_int, alpha: c_float, a: *const c_float, lda: c_int,
        x: *const c_float, incx: c_int, beta: c_float, y: *mut c_float, incy: c_int,
    );
    fn cblas_dgemv(
        layout: c_int, trans: c_int, m: c_int, n: c_int, alpha: c_double, a: *const c_double, lda: c_int,
    ...
    fn cblas_gemm_bf16bf16f32(
        layout: c_int, transa: c_int, transb: c_int, m: c_int, n: c_int, k: c_int, alpha: c_float, a: *const u16,
        lda: c_int, b: *const u16, ldb: c_int, beta: c_float, c: *mut c_float, ldc: c_int,
    );
    ...
    fn cblas_gemm_s8s8s32(
        layout: c_int, transa: c_int, transb: c_int, offsetc: c_int, m: c_int, n: c_int, k: c_int, alpha: c_float,
        a: *const i8, lda: c_int, oa: i8, b: *const i8, ldb: c_int, ob: i8, beta: c_float, c: *mut i32, ldc: c_int,
        co: *const i32,
    );
}
...
extern "C" {
    ...
    pub fn LAPACKE_sgetrs(
        layout: c_int, trans: u8, n: c_int, nrhs: c_int, a: *const c_float, lda: c_int, ipiv: *const c_int,
        b: *mut c_float, ldb: c_int,
    ) -> c_int;
    pub fn LAPACKE_dgetrs(
        layout: c_int, trans: u8, n: c_int, nrhs: c_int, a: *const c_double, lda: c_int, ipiv: *const c_int,
    ...
    pub fn LAPACKE_spotrs(
        layout: c_int, uplo: u8, n: c_int, nrhs: c_int, a: *const c_float, lda: c_int, b: *mut c_float, ldb: c_int,
    ) -> c_int;
    pub fn LAPACKE_sgeqrf(layout: c_int, m: c_int, n: c_int, a: *mut c_float, lda: c_int, tau: *mut c_float) -> c_int;
    pub fn LAPACKE_dgeqrf(layout: c_int, m: c_int, n: c_int, a: *mut c_double, lda: c_int, tau: *mut c_double)
        -> c_int;
}
...
extern "C" {
    pub fn DftiCreateDescriptor(
        handle: *mut DftiDescriptorHandle, precision: c_int, domain: c_int, dimension: c_int, length: c_long,
    ) -> c_long;
...
pub const fn sgemm_nr() -> usize {
    16
}
pub const fn sgemm_mr() -> usize {
    6
}
pub const fn dgemm_nr() -> usize {
    8
}
pub const fn dgemm_mr() -> usize {
    6
}
...
impl core::fmt::Display for MklError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            MklError::ShapeMismatch { a_shape, b_shape } => {
                write!(f, "MKL GEMM shape mismatch: A is {:?}, B is {:?}", a_shape, b_shape)
            }
            MklError::OutputShapeMismatch { expected, got } => {
                write!(f, "MKL GEMM output shape mismatch: expected {:?}, got {:?}", expected, got)
            }
...
fn blas_layout<S: crate::RawData>(view: &crate::ArrayBase<S, crate::Ix2>) -> Option<BlasLayout> {
    ...
    if cs == 1 && (rs >= cols as isize || rows <= 1) {
        return Some(BlasLayout {
            layout: CBLAS_ROW_MAJOR,
            trans: CBLAS_NO_TRANS,
            ld: rs.max(1) as c_int,
        });
    }
...
pub fn sgemm(
    a: ArrayView2<f32>, b: ArrayView2<f32>, mut c: ArrayViewMut2<f32>, alpha: f32, beta: f32,
) -> Result<(), MklError> {
    let (m, k) = a.dim();
    let (kb, n) = b.dim();
    if k != kb {
        return Err(MklError::ShapeMismatch {
            a_shape: a.dim(),
            b_shape: b.dim(),
        });
    }
    if c.dim() != (m, n) {
        return Err(MklError::OutputShapeMismatch {
            expected: (m, n),
            got: c.dim(),
        });
    }
...
pub fn dgemm(
    a: ArrayView2<f64>, b: ArrayView2<f64>, mut c: ArrayViewMut2<f64>, alpha: f64, beta: f64,
) -> Result<(), MklError> {
    let (m, k) = a.dim();
    let (kb, n) = b.dim();
    if k != kb {
...
pub fn sgemm_bf16(
    a: ArrayView2<crate::hpc::quantized::BF16>, b: ArrayView2<crate::hpc::quantized::BF16>, mut c: ArrayViewMut2<f32>,
    alpha: f32, beta: f32,
) -> Result<(), MklError> {
    let (m, k) = a.dim();
    let (kb, n) = b.dim();
...
pub fn sgemm_int8(a: ArrayView2<i8>, b: ArrayView2<i8>, mut c: ArrayViewMut2<i32>) -> Result<(), MklError> {
    let (m, k) = a.dim();
    let (kb, n) = b.dim();
    if k != kb {
        return Err(MklError::ShapeMismatch {
