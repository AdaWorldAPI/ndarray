        ...
        Self(core::array::from_fn(|p| {
            ...
            U32x4(unsafe {
                ...
                if t0 == t1 {
                    ...
                    vandq_u32(z, ternlog_two_input_u32x4(t1, x, y))
                } else if t1 == 0 {
                    vbicq_u32(ternlog_two_input_u32x4(t0, x, y), z)
                } else if t1 == (t0 ^ 0xF) {
                    veorq_u32(z, ternlog_two_input_u32x4(t0, x, y))
                } else if t0 == 0xF {
                    vorrq_u32(vmvnq_u32(z), ternlog_two_input_u32x4(t1, x, y))
                } else if t1 == 0xF {
                    vorrq_u32(z, ternlog_two_input_u32x4(t0, x, y))
                } else {
                    vorrq_u32(
                        vbicq_u32(ternlog_two_input_u32x4(t0, x, y), z),
                        vandq_u32(ternlog_two_input_u32x4(t1, x, y), z),
                    )
                }
            })
        }))
...
#[cfg(target_arch = "aarch64")]
#[inline]
fn ternlog_two_input_u32x4(t: u8, a: uint32x4_t, b: uint32x4_t) -> uint32x4_t {
    // SAFETY: NEON is a baseline feature of every aarch64 target this module compiles
    // for; these are pure register operations on values already in `uint32x4_t`.
    unsafe {
        match t & 0xF {
            0x0 => vdupq_n_u32(0),
            0x1 => vmvnq_u32(vorrq_u32(a, b)),
            0x2 => vandq_u32(vmvnq_u32(a), b),
            0x3 => vmvnq_u32(a),
            0x4 => vbicq_u32(a, b),
            0x5 => vmvnq_u32(b),
            0x6 => veorq_u32(a, b),
            0x7 => vmvnq_u32(vandq_u32(a, b)),
            0x8 => vandq_u32(a, b),
            0x9 => vmvnq_u32(veorq_u32(a, b)),
            0xa => b,
            0xb => vorrq_u32(vmvnq_u32(a), b),
            0xc => a,
