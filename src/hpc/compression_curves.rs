...
fn benchmark_at_dim(dim: usize, n_vectors: usize, n_clusters: usize, cluster_spread: f64) -> Vec<CompressionResult> {
    ...
    let _gt_top10: std::collections::HashSet<usize> = gt_ranked[..10.min(gt_ranked.len())]
        .iter()
        .map(|&(i, _)| i)
        .collect();
    ...
    for &target_bits in &[64, 128, 256, 512, 1024] {
        ...
        let projected: Vec<Vec<u8>> = vectors.iter().map(|v| rp.project_and_binarize(v)).collect();
...
fn native_binary_benchmark(
    ...
    let nodes_s: Vec<Vec<u64>> = (0..n_nodes)
        .map(|i| random_plane(plane_words, i as u64 * 3 + 1))
        .collect();
    let nodes_p: Vec<Vec<u64>> = (0..n_nodes)
        .map(|i| random_plane(plane_words, i as u64 * 3 + 2))
        .collect();
    let nodes_o: Vec<Vec<u64>> = (0..n_nodes)
        .map(|i| random_plane(plane_words, i as u64 * 3 + 3))
        .collect();
...
fn avg_metrics(exact_dists: &[Vec<u32>], bundles: &[Vec<u64>], n_queries: usize, n_nodes: usize) -> (f64, f64) {
    let mut rhos = Vec::new();
    let mut recalls = Vec::new();
    for q in 0..n_queries {
        let gt: Vec<f64> = (0..n_nodes)
            .filter(|&i| i != q)
            .map(|i| exact_dists[q][i] as f64)
            .collect();
        let ap: Vec<f64> = (0..n_nodes)
            .filter(|&i| i != q)
            .map(|i| hamming_u64(&bundles[q], &bundles[i]) as f64)
            .collect();
...
fn avg_metrics_raw(exact_dists: &[Vec<u32>], compressed: &[Vec<u64>], n_queries: usize, n_nodes: usize) -> (f64, f64) {
    let mut rhos = Vec::new();
    let mut recalls = Vec::new();
    for q in 0..n_queries {
        let gt: Vec<f64> = (0..n_nodes)
        ...
        let ap: Vec<f64> = (0..n_nodes)
            .filter(|&i| i != q)
            .map(|i| hamming_u64(&compressed[q], &compressed[i]) as f64)
            .collect();
        rhos.push(spearman(&gt, &ap));
        recalls.push(recall_at_k(&gt, &ap, 10));
    }
...
mod tests {
    ...
    fn compression_curve_128d() {
        ...
        for r in &results {
            eprintln!(
                "║ {:>12} │ {:>6} │ {:>7.1}× │ {:>8.4} │ {:>7.2} │ {:>7.2} ║",
                r.method, r.bits_per_vector, r.compression_ratio, r.spearman_rho, r.recall_at_10, r.cluster_purity
            );
        }
        eprintln!("╚══════════════════════════════════════════════════════════════════════════════╝");
        ...
        for r in &results {
            assert!(r.spearman_rho > -0.5, "{} at {} bits has ρ={:.4}", r.method, r.bits_per_vector, r.spearman_rho);
        }
    }
    ...
    fn compression_curve_768d() {
        ...
        let pq_best = results
            .iter()
            .filter(|r| r.method == "PQ")
            .map(|r| r.spearman_rho)
            .fold(0.0f64, f64::max);
    ...
    fn dimension_separation_sweep() {
        ...
        for r in &results {
            eprintln!(
                "║ {:>4} │ {:>14} │ {:>6} │ {:>9.1} │ {:>9.1} │ {:>5.2} │ {:>6.3} ║",
                r.dim, r.method, r.bits, r.intra_cluster_mean, r.inter_cluster_mean, r.separation_ratio, r.spearman_rho
            );
        }
        eprintln!("╚════════════════════════════════════════════════════════════════════════════════════╝");
        ...
        for method in &["BinaryQuant", "SimHash-256", "SPO-8K"] {
            let method_results: Vec<&DimSweepResult> = results.iter().filter(|r| r.method == *method).collect();
            if method_results.len() >= 2 {
                ...
                eprintln!(
                    "  {} separation: dim={} → {:.2}, dim={} → {:.2}",
                    method,
                    method_results[0].dim,
                    first_sep,
                    method_results.last().unwrap().dim,
                    last_sep
                );
            }
        }
    ...
    fn precision_sweet_spot() {
        ...
        eprintln!("╠══════════════════════════════════════════════════════════════════╣");
        eprintln!("║ {:>7} │ {:>8} │ {:>7} │ {:>12} │ {:>7} ║", "Bits", "ρ", "R@10", "Δρ/bit (×1e6)", "Grade");
        eprintln!("╠══════════════════════════════════════════════════════════════════╣");
        ...
        if !points.is_empty() {
            eprintln!("\n  >>> Sweet spot: {} bits (best marginal ρ gain per bit)", points[peak_idx].bits);
            eprintln!(
                "  >>> Peak ρ: {:.4} at {} bits",
                points.last().unwrap().spearman_rho,
                points.last().unwrap().bits
            );
        }
    }
    ...
    fn head_to_head_equal_budget() {
        ...
        let sh_d: Vec<f64> = (1..n)
            .map(|i| hamming_bytes(&sh_h[0], &sh_h[i]) as f64)
            .collect();
        ...
        let rp_d: Vec<f64> = (1..n)
            .map(|i| hamming_bytes(&rp_h[0], &rp_h[i]) as f64)
            .collect();
        ...
        if dim % 128 == 0 {
            ...
            let codes: Vec<Vec<u8>> = vectors.iter().map(|v| pq.encode(v)).collect();
            let pq_d: Vec<f64> = (1..n)
                .map(|i| pq.asymmetric_dist(&vectors[0], &codes[i]))
                .collect();
    ...
    fn multi_query_precision_curves() {
        ...
        eprintln!("╠══════════════════════════════════════════════════════════════════════╣");
        eprintln!("║ {:>12} │ {:>6} │ {:>8} │ {:>8} │ {:>8} ║", "Method", "Bits", "mean_ρ", "mean_R@10", "std_ρ");
        eprintln!("╠══════════════════════════════════════════════════════════════════════╣");
    ...
    fn absolute_vs_relative_compression() {
        ...
        for r in &sorted {
            ...
            eprintln!(
                "║ {:>12} │ {:>8} │ {:>7.3} │ {:>5.0}× │ {:>8.4} │ {:>7.2} ║",
                r.method, r.bits_per_vector, kb, r.compression_ratio, r.spearman_rho, r.recall_at_10
            );
        }
        eprintln!("╚═══════════════════════════════════════════════════════════════════════════╝");
    ...
    fn native_binary_compression_curve() {
        ...
        let mut sorted = results.iter().collect::<Vec<_>>();
        sorted.sort_by(|a, b| a.method.cmp(&b.method).then(a.bits.cmp(&b.bits).reverse()));

        for r in &sorted {
            let ratio = source_bits as f64 / r.bits as f64;
            eprintln!(
                "║ {:>13} │ {:>6} │ {:>5.1}× │ {:>8.4} │ {:>8.4} │ {:>7.2} │ {:>7.2} ║",
                r.method,
                r.bits,
                ratio,
                r.rho_random,
                r.rho_structured,
                r.recall_at_10_random,
                r.recall_at_10_structured
            );
        }
        eprintln!("╚══════════════════════════════════════════════════════════════════════════════════════════╝");
        ...
        for &bits in &budgets_sorted {
            let bundle = results
                .iter()
                .find(|r| r.method == "CyclicBundle" && r.bits == bits);
            let trunc = results
                .iter()
                .find(|r| r.method == "Truncation" && r.bits == bits);
            let sample = results
                .iter()
                .find(|r| r.method == "BitSample" && r.bits == bits);

            if let (Some(b), Some(t)) = (bundle, trunc) {
                let winner = if b.rho_structured > t.rho_structured {
                    "Bundle"
                } else {
                    "Trunc"
                };
                let s_winner = if let Some(s) = sample {
                    if s.rho_structured > b.rho_structured.max(t.rho_structured) {
                        "Sample"
                    } else {
                        winner
                    }
        ...
        let best_bundle_struct = results
            .iter()
    ...
    fn native_binary_sweet_spot() {
        ...
        for &target_bits in &bit_widths {
            ...
            eprintln!(
                "║ {:>7} │ {:>5.1}× │ {:>8.4} │ {:>7.2} │ {:>8.2} │ {:>11.2}{} ║",
                target_bits,
                source_bits as f64 / target_bits as f64,
                rho,
                recall,
                purity,
                marginal * 1e6,
        ...
        eprintln!("╚══════════════════════════════════════════════════════════════════════╝");
        if sweet_spot > 0 {
            eprintln!("  >>> Sweet spot: {} bits ({}× compression)", sweet_spot, source_bits / sweet_spot);
        }
    }
}
