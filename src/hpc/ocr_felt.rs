...
use super::ocr_simd::{foreground_count, BinaryImage, GrayImage};
...
fn render_synthetic_glyph(c: char) -> Vec<u8> {
    ...
    for y in 1..11 {
        for x in 1..7 {
            ...
            let center_val = ((x as i32 - 3).abs() * 8 + (y as i32 - 5).abs() * 4) as u32;
            let center_bonus = 40u32.saturating_sub(center_val);
...
mod tests {
    ...
    fn test_base17_different_glyphs() {
        ...
        let d_ab = a.l1(&b);
        let d_az = a.l1(&z);
        eprintln!("Base17 L1: A-B={}, A-Z={}", d_ab, d_az);
        assert!(d_ab < 100000, "distances should be finite");
    }
    ...
    fn test_euler_gamma_skew_floor() {
        // γ/(γ+1) ≈ 0.366
        assert!((SKEW_FLOOR - 0.366).abs() < 0.01, "Euler-gamma floor should be ~0.366, got {}", SKEW_FLOOR);
    }
    ...
    fn test_all_three_approaches_comparison() {
        ...
        for &c in &test_chars {
            ...
            eprintln!(
                "  '{}': Base17 → '{}' (d={}, conf={:.2}) | Palette={} | Polar={:016b}",
