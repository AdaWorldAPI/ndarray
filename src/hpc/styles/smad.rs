...
use super::super::nars::{nars_revision, NarsTruth};
...
pub fn debate(input: &Base17, perspectives: &[Base17], rounds: u8) -> DebateResult {
    ...
    for perspective in perspectives {
        ...
        propositions.push(Proposition {
            fingerprint: perspective.clone(),
            truth,
        });
    }

    // Bundle: majority vote per dimension (mean of i16 values)
    let consensus = bundle_base17(
        &propositions
            .iter()
            .map(|p| &p.fingerprint)
            .collect::<Vec<_>>(),
...
mod tests {
    ...
    fn test_debate_consensus() {
        ...
        let perspectives = vec![Base17 { dims: [90; 17] }, Base17 { dims: [110; 17] }, Base17 { dims: [100; 17] }];
    ...
    fn test_debate_truth_accumulates() {
        ...
        let perspectives: Vec<Base17> = (0..5)
            .map(|i| {
