...
pub fn adaptive_style_select(gate_history: &[GateState]) -> StyleRecommendation {
    if gate_history.is_empty() {
        return StyleRecommendation::KeepCurrent;
    }
    ...
    if blocks >= 3 {
        StyleRecommendation::RadicalShift
    } else if blocks >= 1 {
        StyleRecommendation::TryNeighbor
    } else {
        StyleRecommendation::KeepCurrent
    }
}
...
mod tests {
    ...
    fn test_mixed() {
        assert_eq!(
            adaptive_style_select(&[GateState::Flow, GateState::Block, GateState::Flow]),
            StyleRecommendation::TryNeighbor
        );
    }
}
