...
// This is a vendored fork of chacha20 0.9.1 (pre cargo check-cfg): the
// `chacha20_force_{soft,avx2,sse2,neon}` cfgs are real RUSTFLAGS knobs but are
// not declared for check-cfg, so silence the resulting `unexpected_cfgs` note.
